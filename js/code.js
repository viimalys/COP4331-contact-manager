...
function removeLoginErrors() {
	removeFieldError('loginName');
	removeFieldError('loginPassword');
}

function removeRegisterErrors() {
...
function removeAddContactErrors() {
...
function doRegister() {
	...
	if (!firstNameValue || !lastNameValue || !loginValue || !passwordValue || !passwordRequirements.metRequirements
		...
		if (!usernameRegex.test(loginValue) && loginValue) {
...
function addSuccess(first, last) {
	// Contact succcessfully added
	let popup = document.getElementById("landingPopupBox");
	let message = "Contact " + first + " " + last + " successfully added!"
	// Display the popup box
	popup.classList.remove("hidden");
	popup.innerText = message;

	// After 5 seconds, hide the popup box
	setTimeout(function () {
		popup.classList.add("hidden");
	}, 5000);
}

function deleteSuccess(first, last) {
	// Contact succcessfully deleted
	let popup = document.getElementById("landingPopupBox");
	let message = "Contact " + first + " " + last + " successfully deleted!"
	// Display the popup box
	popup.classList.remove("hidden");
	popup.innerText = message
...
function updateSuccess(first, last) {
	// Contact succcessfully updated
	let popup = document.getElementById("landingPopupBox");
	let message = "Contact " + first + " " + last + " successfully updated!"
	// Display the popup box
	popup.classList.remove("hidden");
	popup.innerText = message
...
function copySuccess(first, last) {
	// Contact succcessfully copied
	let popup = document.getElementById("landingPopupBox");
	let message = "Contact " + first + " " + last + " successfully copied to clipboard!"
	// Display the popup box
	popup.classList.remove("hidden");
	popup.innerText = message;
...
function copyFail(first, last) {
	// Contact unsucccessfully copied
	let popup = document.getElementById("landingPopupBox");
	let message = "Contact " + first + " " + last + " was not able to be copied! Clipboard feature is only available in secure contexts (HTTPS)."
	// Display the popup box
	popup.classList.remove("hidden");
	popup.innerText = message;
...
function checkContactValidity() {
	...
	removeAddContactErrors();

	// Check for empty fields + regex
	if (!first || !last || !phone || !email || !nameRegex.test(first) || !nameRegex.test(last) 
		|| !phoneRegex.test(phone) || !emailRegex.test(email)) {
			
		// empty fields
		if (!first) {
			highlightFieldError('firstNameInput', 'First Name cannot be empty.');
		}
		if (!last) {
			highlightFieldError('lastNameInput', 'Last Name cannot be empty.');
		}
		if (!phone) {
			highlightFieldError('phoneInput', 'Phone number cannot be empty.');
		}
		if (!email) {
			highlightFieldError('emailInput', 'Email address cannot be empty.');
		}

		// regex check
		if (!nameRegex.test(first) && first) {
			highlightFieldError('firstNameInput', 'First Name can only contain letters.');
			console.log("first: " + first);
		}
		if (!nameRegex.test(last) && last) {
			highlightFieldError('lastNameInput', 'First Name can only contain letters.');
			console.log("last: " + last);
		}
		if (!phoneRegex.test(phone) && phone) {
			highlightFieldError('phoneInput', 'Phone number is invalid.');
			console.log("phone: " + phone);
		}
		if (!emailRegex.test(email) && email) {
			highlightFieldError('emailInput', 'Email address is invalid.');
...
function addContactRow(firstName, lastName, Phone, Email, contactID) {
	...
	text += "<td id='tableButtons" + contactID + "' class='tableButtons'><button type='button' class='updateButton' onclick='updateContact(" + contactID + ")'><span><img class='tableImages' src='images/edit.svg'></span></button>"
		+ "<button type='button' class='deleteButton' onclick='deleteContact(" + contactID + ")'><span><img class='tableImages' src='images/delete.svg'></span></button>"
		+ "<button type='button' class='copyButton' onclick='copyContact(" + contactID + ")'><span><img class='tableImages' src='images/copy.svg'></span></button></td>";
	text += "</tr>"
...
function addContact() {
	...
	let valid = checkContactValidity();
	console.log(valid);
			...
			xhr.onreadystatechange = function () {
				if (xhr.readyState == 4) {
					if (xhr.status == 200) {
						...
						let jsonIDstr = jsonData.contactId;
						let jsonIDint = parseInt(jsonIDstr, 10);
						addContactRow(firstName, lastName, phone, email, jsonIDint);
						addSuccess(firstName, lastName);
					} else {
						// error handling
					}
				}
	...
	if (confirm("Are you sure you want to delete contact " + firstName + " " + lastName + "?") == true) {
		...
		try {
			xhr.onreadystatechange = function () {
				if (xhr.readyState == 4) {
					if (xhr.status == 200) {
						deleteSuccess(firstName, lastName);
					} else {
						// error handling
					}
				}
...
function copyContact(id) {
	...
	try {
		...
		console.log("Contact " + firstName + " " + lastName + " copied to the clipboard!");
		copySuccess(firstName, lastName);
	} catch (error) {
		console.log("Contact " + firstName + " " + lastName + " was not able to be copied!");
		copyFail(firstName, lastName);
		return;
	}
}

function copyUpdatedContact(id) {
	let firstName = document.getElementById("newFirstName" + id).value;
	let lastName = document.getElementById("newLastName" + id).value;
	let phone = document.getElementById("newPhone" + id).value;
	let email = document.getElementById("newEmail" + id).value;
...
function updateContact(id) {
	...
	tmpRow.innerHTML = "";
	let text = "";
	text += "<td><input type='edit' class='tableInput' id='newFirstName" + id + "' value='" + tmpFirst + "'></input></td>";
	text += "<td><input type='edit' class='tableInput' id='newLastName" + id + "' value='" + tmpLast + "'></input></td>";
	text += "<td><input type='edit' class='tableInput' id='newPhone" + id + "' value='" + tmpPhone + "'></input></td>";
	text += "<td><input type='edit' class='tableInput' id='newEmail" + id + "' value='" + tmpEmail + "'></input></td>";
	text += "<td class='tableButtons'><button type='button' class='saveButton' onclick='saveContact(" + id + ")'><span><img class='tableImages' src='images/save.svg'></span></button>"
		+ "<button type='button' class='deleteButton' onclick='deleteContact(" + id + ")'>" + "<span><img class='tableImages' src='images/delete.svg'></span></button>"
		+ "<button type='button' class='copyButton' onclick='copyUpdatedContact(" + id + ")'><span><img class='tableImages' src='images/copy.svg'></span></button></td>";
...
function saveContact(id) {
	...
	text += "<td id='tableButtons" + id + "' class='tableButtons'><button type='button' class='updateButton' onclick='updateContact(" + id + ")'><span><img class='tableImages' src='images/edit.svg'></span></button>"
		+ "<button type='button' class='deleteButton' onclick='deleteContact(" + id + ")'><span><img class='tableImages' src='images/delete.svg'></span></button>"
		+ "<button type='button' class='copyButton' onclick='copyContact(" + id + ")'><span><img class='tableImages' src='images/copy.svg'></span></button></td>";
	...
	try {
		xhr.onreadystatechange = function () {
			if (xhr.readyState == 4) {
				if (xhr.status == 200) {
					updateSuccess(savedFirst, savedLast);
				} else {
					// error handling
				}
			}
				...
				let jsonData = JSON.parse(xhr.responseText);
					...
					for (let i = 0; i < jsonData.results.length; i++) {
						...
						if (userId == jsonUserIDint) {
							addContactRow(jsonData.results[i].FirstName, jsonData.results[i].LastName, jsonData.results[i].Phone, jsonData.results[i].Email, jsonData.results[i].ID);
						}
					}
			}
		};
